...
=======================
- You are not restricted to only academic tutoring. If the student wants to have a normal, casual conversation — greetings, small talk, jokes, general knowledge, life advice, opinions, or anything outside formal academics — respond naturally, warmly, and helpfully like a friendly, intelligent companion would.
- The SUBJECT CONVERGENCE and SYLLABUS restrictions below apply ONLY when the student is asking an academic/study-related question. They do NOT apply to casual, non-academic conversation.
- Use good judgment: if a message is clearly not a study question, just chat normally instead of redirecting the student back to their syllabus or active subject.
...
- Do NOT provide fictional or mock video descriptions. Only present factual findings.`;
    ...
    // Temporarily disabled googleSearch tool for debugging 403 errors.
    // If this works without tools, the issue is Google Search grounding
    // permission on the free tier API key (which requires paid billing).
    ...
    const rawDetail = (error?.message || error?.error?.message || String(error) || "").toString().slice(0, 300);

    // Full raw error, always logged, so Vercel Function Logs show the actual
    // Google API response instead of just a generic status classification.
    console.error("[Gemini RAW ERROR /api/chat]", {
      message: error?.message,
      status: error?.status,
      code: error?.code,
      errorObjCode: error?.error?.code,
      errorObjStatus: error?.error?.status,
      errorObjMessage: error?.error?.message,
      name: error?.name
    });
    ...
    if (isAuthError) {
      ...
      return res.status(403).json({ 
        error: `Gemini API Connection Failed: The configured API key is invalid or lacks required permissions. Details: ${rawDetail || "no further detail from Google API."}` 
      });
    }
    ...
    if (isQuotaError) {
      ...
      return res.status(429).json({ 
        error: `Gemini API Quota Exceeded: Daily model rate limits have been reached for this API key. If using a free key, consider upgrading or selecting a billing-enabled key in Settings > Secrets. Details: ${rawDetail || "no further detail from Google API."}` 
      });
    }
...
app.post("/api/generate-quiz", async (req, res) => {
  const { subject, topic, userClass, userBoard, userSyllabus } = req.body;
  const cleanSubject = sanitizeString(subject, 150);
  const cleanTopic = sanitizeString(topic, 150);
  const cleanClass = sanitizeString(userClass, 60);
  const cleanBoard = sanitizeString(userBoard, 60);
  ...
  try {
    const quizSyllabusConstraint = cleanClass && cleanBoard ? `
    CRITICAL SYLLABUS BOUNDARY (MANDATORY):
    - The student is in Class/Grade "${cleanClass}", under Board/Exam Track "${cleanBoard}".
    ${userSyllabus ? `- Student's Allowed Syllabus Details (by subject): ${JSON.stringify(userSyllabus)}` : ""}
    - Every question, option, and explanation MUST use ONLY concepts, terminology, and difficulty level that fall strictly within the official syllabus for "${cleanClass}" under "${cleanBoard}".
    - NEVER include a concept, formula, or terminology that belongs to a higher grade, a different board, or a topic outside "${cleanTopic}" as officially covered at this level.
    - If a concept is ambiguous or could be interpreted at multiple difficulty levels, default to the simplest interpretation that fits "${cleanClass}".` : "";
    ...
    ${quizSyllabusConstraint}

    MANDATORY SYLLABUS SELF-VERIFICATION (perform silently before finalizing your output):
    For EACH question you draft, check:
    1. Does this question rely on any concept, formula, or terminology that is taught in a LATER grade/class than "${cleanClass || "the student's level"}"?
    2. Does this question belong to a different subject or chapter than "${cleanTopic}"?
    3. Would a student who has only studied the official "${cleanClass}" "${cleanBoard}" syllabus for this topic be able to reasonably attempt this question using only what they've been taught?
    If the answer to (1) or (2) is YES, or the answer to (3) is NO, DISCARD that question and silently regenerate a simpler, syllabus-compliant replacement before returning your final output. Never surface this checklist or your reasoning about it in the output - only return the final validated question set.
...
app.post("/api/generate-flashcards", async (req, res) => {
  ...
  try {
    const flashcardSyllabusConstraint = cleanClass && cleanBoard ? `
    CRITICAL SYLLABUS BOUNDARY (MANDATORY):
    - The student is in Class/Grade "${cleanClass}", under Board/Exam Track "${cleanBoard}".
    ${userSyllabus ? `- Student's Allowed Syllabus Details (by subject): ${JSON.stringify(userSyllabus)}` : ""}
    - Every flashcard's question and answer MUST use ONLY concepts, terminology, and difficulty level that fall strictly within the official syllabus for "${cleanClass}" under "${cleanBoard}".
    - NEVER include a concept, formula, or terminology that belongs to a higher grade, a different board, or a topic outside "${cleanTopic}" as officially covered at this level.
    - If a concept is ambiguous or could be interpreted at multiple difficulty levels, default to the simplest interpretation that fits "${cleanClass}".` : "";
    ...
    ${flashcardSyllabusConstraint}

    MANDATORY SYLLABUS SELF-VERIFICATION (perform silently before finalizing your output):
    For EACH flashcard you draft, check:
    1. Does this card rely on any concept, formula, or terminology that is taught in a LATER grade/class than "${cleanClass || "the student's level"}"?
    2. Does this card belong to a different subject or chapter than "${cleanTopic}"?
    3. Would a student who has only studied the official "${cleanClass}" "${cleanBoard}" syllabus for this topic be able to reasonably understand this card using only what they've been taught?
    If the answer to (1) or (2) is YES, or the answer to (3) is NO, DISCARD that card and silently regenerate a simpler, syllabus-compliant replacement before returning your final output. Never surface this checklist or your reasoning about it in the output - only return the final validated flashcard set.
