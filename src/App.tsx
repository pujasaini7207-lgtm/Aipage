...
export default function App() {
  ...
  const [weeklyActivity, setWeeklyActivity] = useState<Record<string, number>>(() => {
    if (typeof window !== "undefined") {
      try {
        return JSON.parse(localStorage.getItem("lunito_weekly_activity") || "{}");
      } catch {
        return {};
      }
    }
  ...
  const registerChatSent = async () => {
    ...
    const updatedWeekly = { ...weeklyActivity, [todayKey]: (weeklyActivity[todayKey] || 0) + 1 };
    ...
    if (isGuest) {
      ...
      localStorage.setItem("lunito_weekly_activity", JSON.stringify(updatedWeekly));
      return;
    }
    ...
    if (auth.currentUser) {
      const userDocRef = doc(db, "users", auth.currentUser.uid);
      try {
        await updateDoc(userDocRef, { chatsToday: nextCount, weeklyActivity: updatedWeekly });
      } catch (err) {
  ...
  const safeFetch = async (input: RequestInfo | URL, init?: RequestInit): Promise<Response> => {
    ...
    const contextInjectedEndpoints = [
      "/api/chat",
    ...
    if (typeof input === "string" && contextInjectedEndpoints.includes(input) && customInit && customInit.body) {
      try {
      ...
      if (user) {
        ...
        try {
          ...
          if (docSnap.exists()) {
            ...
            if (userData.weeklyActivity !== undefined) setWeeklyActivity(userData.weeklyActivity);
  ...
  const loginWithGoogle = async () => {
    ...
    try {
      ...
      // If result.user is null, a real redirect to Google's account picker
      // is in progress - the browser is about to navigate away, so no
        ...
        <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-[#07070a] via-[#0E122A] to-[#07070a] px-4 py-6">
          <div className="w-full max-w-sm p-5 md:p-8 rounded-2xl border border-zinc-800 bg-zinc-950/90 backdrop-blur-2xl shadow-xl shadow-black/80 relative overflow-hidden">
            ...
            <div className="space-y-4 font-sans">
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    if (newClass === "JEE Aspirant") {
                      setSelectedBoard("JEE (Main & Advanced)");
                      setProfileSubjects(["Physics", "Chemistry", "Mathematics"]);
                    } else if (newClass === "NEET Aspirant") {
                      setSelectedBoard("NEET UG");
                      setProfileSubjects(["Physics", "Chemistry", "Biology"]);
                    } else if (selectedBoard === "JEE (Main & Advanced)" || selectedBoard === "NEET UG") {
                ...
                >
                  {["Class 6", "Class 7", "Class 8", "Class 9", "Class 10", "Class 11", "Class 12", "JEE Aspirant", "NEET Aspirant", "Undergraduate", "Postgraduate", "Other"].map((cls) => (
                    <option key={cls} value={cls} className="bg-zinc-950 text-white font-sans">{cls}</option>
                  ))}
                </select>
              </div>

              {selectedClass !== "JEE Aspirant" && selectedClass !== "NEET Aspirant" && (
                <div>
              ...
              {(selectedClass === "JEE Aspirant" || selectedClass === "NEET Aspirant") && (
                <div className="px-3.5 py-2.5 rounded-xl bg-violet-500/10 border border-violet-500/20 text-[11px] text-violet-300 font-semibold leading-relaxed">
                  Preparing for {selectedClass === "JEE Aspirant" ? "JEE (Main & Advanced)" : "NEET UG"}. Your syllabus, quizzes, and notes will be tailored to the official {selectedClass === "JEE Aspirant" ? "JEE" : "NEET"} exam pattern.
                </div>
              )}
